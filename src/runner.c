...
static bool isObjectDisabled(Runner* runner, int32_t objectIndex) {
    if (runner->disabledObjects == NULL) return false;
...
Instance* Runner_createInstance(Runner* runner, GMLReal x, GMLReal y, int32_t objectIndex) {
    if (isObjectDisabled(runner, objectIndex)) return NULL;
...
void Runner_destroyInstance(__attribute__((unused)) Runner* runner, Instance* inst) {
